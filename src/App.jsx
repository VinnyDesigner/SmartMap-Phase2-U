                ...
                <div className="map-ai-panel-main-grid" style={{ display: 'flex', width: '100%', height: '100%', gap: '16px', margin: 0, padding: 0 }}>
                  ...
                  {isAISearchBarOpen && selectedLocation && (
                    <div className="map-ai-panel-right-col" style={{
                      ...
                      minWidth: panelHeight <= 100 ? 0 : '380px',
                      padding: panelHeight <= 100 ? 0 : '16px',
                      borderWidth: panelHeight <= 100 ? 0 : '1px',
                      marginTop: panelHeight <= 100 ? 0 : '48px',
                      height: panelHeight <= 100 ? 0 : 'calc(100% - 48px)',
                      overflow: 'hidden',
                      transition: 'all 0.25s cubic-bezier(0.16, 1, 0.3, 1)'
                    }}>
                      ...
                      <div className="detailed-info-name-row">
                        <div>
                          <h3 className="detailed-info-name">{selectedLocation.title}</h3>
                          {selectedLocation.arabicTitle && (
                            <div className="detailed-info-arabic">{selectedLocation.arabicTitle}</div>
                          )}
                        </div>
                        <div className="detailed-info-quick-actions">
                          <button
                            ...
                            title="Favorite"
                            onClick={() => {
                              selectedLocation.isFavorite = !selectedLocation.isFavorite;
                              setSelectedLocation({ ...selectedLocation });
                              showToast(selectedLocation.isFavorite ? "Removed from Favorites" : "Saved to Favorites");
                            }}
                          >
                          ...
                          <button
                            ...
                            onClick={() => {
                              if (selectedLocation) {
                                setSelectedLocation({ ...selectedLocation, locateTrigger: Date.now() });
                              }
                              showToast(`Located ${selectedLocation.title} on Map`);
                            }}
                          >
                            <Search size={14} color="#64748B" />
                          </button>
                      ...
                      {activeDetailTab === 'overview' ? (
                        <div className="detailed-info-body">
                          <div className="detailed-info-contact-item">
                          ...
                          <div className="detailed-info-contact-item">
                            ...
                            <a href={`https://${selectedLocation.title.toLowerCase().replace(/[^a-z]/g, '')}.sch.ae`} target="_blank" rel="noreferrer" style={{ color: '#2563EB', textDecoration: 'none' }}>
                              https://{selectedLocation.title.toLowerCase().replace(/[^a-z]/g, '')}.sch.ae
                            </a>
                          </div>
                          ...
                          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                            <div className="detailed-info-contact-item">
                              ...
                              <span>info@{selectedLocation.title.toLowerCase().replace(/[^a-z]/g, '')}.sch.ae</span>
                            </div>
                            <div className="detailed-info-contact-item">
                              <Phone size={14} color="#2563EB" />
                              <span>+971 2 815 6500</span>
                            </div>
                          </div>

                          <div className="detailed-info-ai-reason">
                            <div className="detailed-info-ai-reason-title">
                              <Sparkles size={14} />
                              <span>Here's why this is a good option for you</span>
                            </div>
                            <div className="detailed-info-chips-grid">
                              <span className="detailed-info-badge-chip">Academic Year: 2024</span>
                              <span className="detailed-info-badge-chip">Average Tuition Fee: 28,912 AED</span>
                              <span className="detailed-info-badge-chip">IRTQAA Result: Very Good</span>
                            </div>
                          </div>
                        </div>
                      ) : (
                        <div className="detailed-info-body">
                          <div style={{ background: '#F8FAFC', padding: '10px', borderRadius: '8px', border: '1px solid #F1F5F9' }}>
                            <div style={{ fontWeight: '700', color: '#0F172A', marginBottom: '4px' }}>Category & Subcategory</div>
                            <div>{selectedLocation.category} &bull; {selectedLocation.subcategory}</div>
                          </div>
                          <div style={{ background: '#F8FAFC', padding: '10px', borderRadius: '8px', border: '1px solid #F1F5F9' }}>
                            <div style={{ fontWeight: '700', color: '#0F172A', marginBottom: '4px' }}>Description</div>
                            <div>{selectedLocation.description}</div>
                          </div>
                          <div style={{ background: '#F8FAFC', padding: '10px', borderRadius: '8px', border: '1px solid #F1F5F9' }}>
                            <div style={{ fontWeight: '700', color: '#0F172A', marginBottom: '4px' }}>Geospatial Coordinates</div>
                            <div>Latitude: {selectedLocation.lat} &bull; Longitude: {selectedLocation.lon}</div>
                          </div>
                        </div>
                      )}
                    </div>
